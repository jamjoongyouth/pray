...
2. **패들릿을 삭제하거나 프라이버시를 "비공개"로 바꾸지 마세요.** 바꾸면
   "교제내용 보러가기" 버튼이 죽습니다. 프라이버시는 **"비밀(링크 아는 사람만)"**,
   방문자 권한은 **"작성 가능"** 이어야 로그인 없이 답을 올릴 수 있습니다.

## 콘텐츠 수정 방법

| 무엇을 | 어떻게 |
|---|---|
| 교제내용 질문 | 패들릿의 각 회차 컬럼 맨 위 **질문 카드**를 수정 (즉시 반영, 재배포 불필요) |
| 올라온 답 내리기 | 패들릿에서 해당 게시물의 `⋯` → 삭제. 리더(소유자)만 남의 글을 지울 수 있음 |
| 교제내용 링크 자체 | `index.html`의 `.fellowship` 섹션 `href` 수정 후 push |

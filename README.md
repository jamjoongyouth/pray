...
2. **`nanum.html`의 파일명을 바꾸지 마세요.** 인쇄한 질문지의 QR이 이 주소를
   가리킵니다. 이름이 바뀌면 나눠준 종이의 QR이 전부 죽습니다.
...
| 무엇을 | 어떻게 |
|---|---|
| 히든 질문·인도 안내 | `nanum.html`의 해당 `.paper-sec` 안 문구 수정 후 push (주소가 그대로라 QR은 계속 유효) |
| 히든 질문 추가 | `nanum.html`에 `.paper-sec` 블록을 하나 더 추가. 같은 페이지이므로 QR은 다시 만들 필요 없음 |
